...
from app_blog.models import Category, Post, Comment

register = template.Library()


@register.inclusion_tag('app_blog/_blog-category-section.html')
...
@register.inclusion_tag('app_blog/_blog-author-section.html')
def author_section():
...
@register.inclusion_tag('app_blog/_blog-latest-posts-section.html')
def latest_posts(number=3):
    posts = Post.objects.filter(status=1).order_by('-published_date')[:number]
    return {'posts': posts}


@register.inclusion_tag('app_blog/_blog-last-3-posts.html')
def last_3_posts():
...
@register.inclusion_tag('app_blog/_blog-tag-section.html')
def tag_section():
    tags = []
